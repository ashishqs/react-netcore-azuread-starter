...
var processAdalCallback = function() {
  ...
  if (_adal.isCallback(hash)) {
    ...
    if (requestInfo.requestType !== _adal.REQUEST_TYPE.LOGIN) {
      if ((window as any).parent.AuthenticationContext === 'function' && (window as any).parent.AuthenticationContext()) {
        _adal.callback = (window as any).parent.AuthenticationContext().callback;
      }
      if (requestInfo.requestType === _adal.REQUEST_TYPE.RENEW_TOKEN) {
        _adal.callback =(window as any).parent.callBackMappedToRenewStates[requestInfo.stateResponse];
      }
    }
      ...
      if (typeof _adal.callback === 'function') {
        ...
        if (_oauthData.userName) {
          ...
          window.setTimeout(function() {
            ...
            if (loginStartPage) {
              (window as any).location.path(loginStartPage);
            }
          }, 1);
        }
      }
...
export   {adalRequest, processAdalCallback};